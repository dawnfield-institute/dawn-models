...
class GAIA:
    ...
    def run_benchmarks():
        """
        Run built-in GAIA benchmarks: collapse throughput, entropy reduction, symbolic emergence, thermodynamic cost.
        """
        print("\n=== GAIA Benchmark Suite ===")
        gaia = GAIA(field_shape=(32, 32), collapse_threshold=0.6)

        # Benchmark 1: Collapse Throughput
        print("\n[1] Collapse Throughput Benchmark")
        n_steps = 100
        start = time.time()
        collapse_counts = []
        for _ in range(n_steps):
            state = gaia.step()
            collapse_counts.append(state.total_collapses)
        elapsed = time.time() - start
        print(f"Steps: {n_steps}, Time: {elapsed:.3f}s, Collapses: {collapse_counts[-1]}, Throughput: {collapse_counts[-1]/elapsed:.2f} collapses/sec")

        # Benchmark 2: Entropy Reduction
        print("\n[2] Entropy Reduction Benchmark")
        gaia.reset()
        initial_entropy = gaia.field_engine.get_field_state().field_pressure
        for _ in range(n_steps):
            gaia.step()
        final_entropy = gaia.field_engine.get_field_state().field_pressure
        print(f"Initial entropy: {initial_entropy:.4f}, Final entropy: {final_entropy:.4f}, ΔEntropy: {initial_entropy-final_entropy:.4f}")

        # Benchmark 3: Symbolic Emergence
        print("\n[3] Symbolic Emergence Benchmark")
        gaia.reset()
        for _ in range(n_steps):
            gaia.step()
        summary = gaia.get_symbolic_summary()
        print(f"Total structures: {summary['total_structures']}, Types: {summary.get('collapse_type_counts',{})}")

        # Benchmark 4: Thermodynamic Cost Tracking
        print("\n[4] Thermodynamic Cost Benchmark")
        total_cost = summary.get('total_thermodynamic_cost', 0.0)
        print(f"Total thermodynamic cost: {total_cost:.4f}")
